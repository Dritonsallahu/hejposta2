...
class WaitingSpecificOrders extends StatefulWidget {
  ...
  final String? status;
  WaitingSpecificOrders({Key? key, this.orders, required this.status})
      : super(key: key);
    ...
    postmanOrdersController.pranoPorosine(context, id).then((value) async {
      if (value == "success") {
        if (widget.orders!.length == 1) {
          ...
          setState(() {
            widget.orders!.removeWhere((element) => element.orderNumber == id);
          });
        }
        if(isScanning && widget.orders!.length > 1){
          String barcodeScanRes = await FlutterBarcodeScanner.scanBarcode(
              "#ff6666", "Largo", true, ScanMode.QR)
              .then((value) {
            return value;
          });
          print("brcode status: ${barcodeScanRes.isNotEmpty} - ${barcodeScanRes}");
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "Produkti: ${order.orderName!}",maxLines: 1,
                                    style: AppStyles.getHeaderNameText(
                                        color: AppColors.orderDescColor,
                                        size: 16.0),
                                  ),
                                  Text(
                                    "Adresa: ${order.receiver!.address!}",maxLines: 2,
                                    style: AppStyles.getHeaderNameText(
                                        color: AppColors.orderDescColor,
                                        size: 16.0),
                                  ),
                          ...
                          Stack(
                            children: [
                              Container(
                                ...
                                child: Center(
                                  child: Text(
                                    getOrderStatus(widget.orders![index].status
                                        .toString()),
                                    textAlign: TextAlign.center,
                                    style: AppStyles.getHeaderNameText(
                                        color: Colors.white, size: 15.0),
